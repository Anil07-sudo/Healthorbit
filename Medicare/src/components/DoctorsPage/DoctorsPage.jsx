...
import { Search, X, Medal, ChevronRight,CircleChevronDown,CircleChevronUp } from 'lucide-react'; // Added missing Medal and ChevronRight
...
const DoctorsPage = () => {
  ...
  // Helper to normalize doctor data (used in both useEffect and retry)
  ...
  const loadDoctors = async (isMounted) => {
    setLoading(true);
    setError("");
    try {
      ...
      if (isMounted) {
        setAllDoctors(normalizeData(items));
      }
    } catch (err) {
      if (isMounted) {
